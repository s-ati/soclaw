...
class _Renderer:
    ...
    def draw_evidence(self):
        ...
        blocks = [
            ("Skills Detected",
             ", ".join(ext.get("skills", [])) or "No strong skill signals detected"),
            ("Coursework Signals",
             ", ".join(ext.get("courses", [])) or "None identified"),
            ("Project Evidence",
             "; ".join(ext.get("projects", [])[:3]) or "None detected via keyword scan"),
            ("Work-Style Source",
        ...
        for label, content in blocks:
            safe = _esc(content)
            self._para(f"<b>{_esc(label)}:</b>&nbsp;&nbsp;{safe}",
                        S_EVIDENCE)
            self._gap(3)
